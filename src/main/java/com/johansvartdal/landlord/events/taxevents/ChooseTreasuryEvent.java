...
public class ChooseTreasuryEvent extends LandlordEvent {
    ...
    private void chooseRandom() {
        ...
        List<? extends Player> playingPlayers = onlinePlayers.stream().filter(player -> {
            return Main.playerDataManager.playerExists(player); // skip spectators
        }).toList();

        int randomNumber = random.nextInt(playingPlayers.size());

        Player chosenPlayer = playingPlayers.get(randomNumber);
