...
public class Fly implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (Tools.stateNotNormal(player) || JailManager.playerIsInJail(player) || !Main.playerDataManager.playerExists(player)) {
	...
	private void attemptFlying(Player player) {
		...
		if (!Bank.playerCanAfford(player, StaticValues.FLYING_PRICE_PER_MINUTE)) {
			Bank.tellPlayerCannotAfford(player, LangDict.getString("playerEvents.fly.flying"), StaticValues.FLYING_PRICE_PER_MINUTE);
			return;
		}
	...
	private void attemptEndFlight(Player player) {
		...
		Tools.tellPlayer(player, LangDict.getString("playerEvents.fly.flightTurnedOff"), ChatColor.GREEN);
	}
}
