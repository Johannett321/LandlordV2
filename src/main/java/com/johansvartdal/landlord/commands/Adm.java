...
import com.johansvartdal.landlord.events.adventure.*;
...
public class Adm implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        if (strings.length == 0) {
            ...
            Tools.printMenuOption(player, "/adm", "rain");
            ...
            Tools.printMenuOption(player, "/adm", "chunkcircle");
            Tools.printMenuOption(player, "/adm", "hint");
            Tools.printMenuOption(player, "/adm", "music");
            return true;
        }
        ...
        if (strings[0].equals("clear")) {
        ...
        }else if (strings[0].equals("rain")) {
            player.getWorld().setTime(0);
            player.getWorld().setStorm(true);
            player.getWorld().setWeatherDuration((int) Tools.secToTicks(30));
            ...
            if (strings.length == 1) {
                Tools.printMenuHeader(player, "Available events");
                Tools.printMenuOption(player, "/adm", "testevent valleyvillage");
                Tools.printMenuOption(player, "/adm", "testevent icyhills");
                Tools.printMenuOption(player, "/adm", "testevent lumbermine");
                Tools.printMenuOption(player, "/adm", "testevent mesa");
                Tools.printMenuOption(player, "/adm", "testevent sibiriancottage");
                return true;
            }
            switch (strings[1]) {
                case "valleyvillage":
                ...
                case "icyhills":
                    ...
                    break;
                case "lumbermine":
                    LandlordEventManager.startEvent(new LumberMineForestAdventure(plugin));
                    break;
                case "mesa":
                    LandlordEventManager.startEvent(new MesaHillsideAdventure(plugin));
                    break;
                case "sibiriancottage":
                    LandlordEventManager.startEvent(new SibirianCottageWinterAdventure(plugin));
                    break;
                default:
                    Tools.tellPlayer(new ErrorChat(), player, "Unknown event: " + strings[1]);
                    break;
            }
        ...
        }else if (strings[0].equalsIgnoreCase("chunkcircle")) {
            GameJustStarted gameJustStarted = new GameJustStarted(plugin, player.getWorld());
            gameJustStarted.createCirclesAroundPlayer(player);
        }else if (strings[0].equalsIgnoreCase("hint")) {
            RandomHint randomHint = new RandomHint(plugin);
            randomHint.showHint();
        }else if (strings[0].equalsIgnoreCase("music")) {
            // Get the hopper block at the specified location
            Location location = new Location(Bukkit.getWorld("lladv"), 101, 70, -875);
            Block hopperBlock = location.getBlock();

            // Set the block to a hopper if it isn't one already
            if (hopperBlock.getType() != Material.HOPPER) {
                hopperBlock.setType(Material.HOPPER);
            }

            debugLog("HOPPER BLOCK TYPE: " + hopperBlock.getType());
            if (hopperBlock.getState() instanceof Hopper hopper) {
                debugLog("Correct instance");
                ItemStack stalDisc = new ItemStack(Material.MUSIC_DISC_STAL, 1);
                hopper.getInventory().setItem(0, stalDisc);
            }else {
                debugLog("Incorrect instance");
            }

            Bukkit.getScheduler().runTaskLater(plugin, () -> {
                hopperBlock.setType(Material.AIR);
            }, Tools.secToTicks(3));
        }
        return true;
    }
