...
public class Capture implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] args) {
        ...
        if (args.length == 0) {
            ...
            Tools.printMenuOption(player, "/capture", "now " + "(" + Tools.formatCurrency(StaticValues.CAPTURE_PRICE) + ")");
            Tools.printMenuOption(player, "/capture", "info");
            return true;
        }

        if (args[0].equalsIgnoreCase("info")) {
            Tools.printMenuHeader(player, LangDict.getString("capture.infoHeader"));
            Tools.printMenuOption(player, LangDict.getString("capture.animalPrice"), Tools.formatCurrency(StaticValues.CAPTURE_PRICE));
            Tools.printMenuOption(player, LangDict.getString("capture.villagerPrice"), Tools.formatCurrency(StaticValues.VILLAGER_CAPTURE_PRICE));
            return true;
        }
    ...
    private void attemptCapture(Player player) {
        ...
        if (near.get(entityIndexLowest).getType() == EntityType.COW) {
            ...
            if (Bank.playerCanAfford(player, StaticValues.VILLAGER_CAPTURE_PRICE)) {
                priceToWithdraw = StaticValues.VILLAGER_CAPTURE_PRICE;
